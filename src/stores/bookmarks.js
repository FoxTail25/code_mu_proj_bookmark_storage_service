...
export const useBookmarkStore = defineStore('bookmarks', () => {
  ...
  function changeRecordOrder(groupId, recordId, direct) {
    ...
    if (direct == 'up') {
      bookmarkArr.value
        .filter((e) => e.id == groupId)[0]
        .bookmarksList.forEach((elem, index) => {
          if (elem.id == recordId) {
            currentIndex = index
            replaceableIndex = index - 1
          }
    ...
    bookmarkArr.value.filter((e) => e.id == groupId)[0].bookmarksList[currentIndex].linkOrder =
      replaceableIndex
    bookmarkArr.value.filter((e) => e.id == groupId)[0].bookmarksList[replaceableIndex].linkOrder =
      currentIndex
    bookmarkArr.value
      .filter((e) => e.id == groupId)[0]
      .bookmarksList.sort((a, b) => a.linkOrder - b.linkOrder)
  ...
  function addNewLinkRecord(groupId, newRecordObjData) {
    let newLinkRecord = {
      ...newRecordObjData,
      id: nanoid(),
      linkOrder: bookmarkArr.value.filter((e) => e.id == groupId)[0].bookmarksList.length,
      edit: false,
    }
  ...
  function restoreDeletedLinkInSelectedGroup(groupId, linkId) {
    ...
    this.deletedLinkArr.forEach((e, i) => {
      if (e.id == linkId) {
        deletedLinkIndex = i;
      };
    });
    let deletedLink = deletedLinkArr.value.splice(deletedLinkIndex, 1)[0];
    let deletedLinkOrder = [...bookmarkArr.value].filter(e => e.id == groupId)[0].bookmarksList.length;
    deletedLink.linkOrder = deletedLinkOrder;
    let groupIndex;
    bookmarkArr.value.forEach((e,i) => {
      if(e.id == groupId) {
        groupIndex = i;
    ...
    bookmarkArr.value[groupIndex].bookmarksList.push(deletedLink);
    localStorageWork.setRecord([...bookmarkArr.value])
  ...
  return {
    ...
    restoreDeletedLinkInSelectedGroup,
  }
})
